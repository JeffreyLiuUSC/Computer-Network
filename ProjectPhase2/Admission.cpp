/*
 * Admission.cpp
 * Serve as TCP server and UDP talker
...
// Main program entry
int main() {
    create_empty_database_file(); // initialize a local .txt file to store the data from departments and students
    create_tcp_and_process(); // create socket
    make_admission_decision(); // making admission decisions
    create_udp_and_process(); // Using UDP to notify students and departments
  return 0;
}
...
void create_tcp_and_process() {
  ...
  sockfd = Socket::create_socket(TCP_SERVER); //created in Socket.cpp including creation setsocketopt and binding
