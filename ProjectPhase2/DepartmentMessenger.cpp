/*
 * DepartmentMessenger.cpp
 * Helper class of Department class
 */
...
void DepartmentMessenger::display(int type, std::vector<std::string> *args) {
  ...
  switch(type) {
    ...
    case DMSG_ADM_CONNECTED:
      std::cout << "Department" << d
                << " is now connected to the admission office\n";
      break;
    case DMSG_PROG_SENT:
      std::cout << "Department" << d
                << " has sent " << args->at(0)
    ...
    case DMSG_P2_UDP_START:
      std::cout << "Department" << d
                << " has UDP port " << args->at(1)
                << " and IP " << args->at(0)
                << "\n";
      break;
    case DMSG_STUDENT_ADMITTED:
      std::cout << args->at(0) << " has been admitted to Department"
                << d
                << "\n";
      break;
    case DMSG_P2_END:
